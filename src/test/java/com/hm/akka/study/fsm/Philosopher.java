...
import static com.hm.akka.study.fsm.Messages.Put;
import static com.hm.akka.study.fsm.Messages.Take;
import static com.hm.akka.study.fsm.Messages.Think;
...
public class Philosopher extends AbstractLoggingFSM<Philosopher.HS, Philosopher.TakenChopsticks> {
...

	{
		...
		when(HS.Hungry, matchEvent(Messages.Taken.class, (taken, data) -> taken.chopstick == left, (taken, data) -> goTo(HS.WaitForOtherChopstick).using(new TakenChopsticks(left, null)))
				.event(Messages.Taken.class, (taken, data) -> taken.chopstick == right, (taken, data) -> goTo(HS.WaitForOtherChopstick).using(new TakenChopsticks(null, right))).event(Messages.Busy.class, (busy, data) -> goTo(HS.FirstChopstickDenied)));

		// When a hakker is waiting for the last chopstick it can either obtain it
		// and start eating, or the other chopstick was busy, and the hakker goes
		// back to think about how he should obtain his chopsticks :-)
		when(HS.WaitForOtherChopstick, matchEvent(Messages.Taken.class, (taken, data) -> (taken.chopstick == left && data.left == null && data.right != null), (taken, data) -> startEating(left, right))
				.event(Messages.Taken.class, (taken, data) -> (taken.chopstick == right && data.left != null && data.right == null), (taken, data) -> startEating(left, right)).event(Messages.Busy.class, (busy, data) -> {
					if (data.left != null)
						left.tell(Put, self());
					if (data.right != null)
						right.tell(Put, self());
		...
		when(HS.FirstChopstickDenied, matchEvent(Messages.Taken.class, (taken, data) -> {
			taken.chopstick.tell(Put, self());
			return startThinking(Duration.create(10, MILLISECONDS));
		}).event(Messages.Busy.class, (busy, data) -> startThinking(Duration.create(10, MILLISECONDS))));
