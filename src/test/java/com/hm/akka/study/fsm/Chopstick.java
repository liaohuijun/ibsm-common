...
public class Chopstick extends AbstractLoggingFSM<Chopstick.CS, Chopstick.TakenBy> {
	{
		...
		when(CS.Available, matchEventEquals(Take, (take, data) -> goTo(CS.Taken).using(new TakenBy(sender())).replying(new Messages.Taken(self()))));

		// When a chopstick is taken by a hakker
		// It will refuse to be taken by other hakkers
		// But the owning hakker can put it back
		when(CS.Taken, matchEventEquals(Take, (take, data) -> stay().replying(new Messages.Busy(self()))).event((event, data) -> (event == Put) && (data.hakker == sender()), (event, data) -> goTo(CS.Available).using(new TakenBy(context().system().deadLetters()))));
