...
//		generationData();
	...
	private static JSONArray loadCollectionData(MongoDatabase db, String collectionName) throws ParseException {
		Date start = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse("2017-10-11 00:00:00");
		Date end = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse("2017-10-12 00:00:00");
		AggregateIterable<Document> iterable = db.getCollection(collectionName)
				.aggregate(Arrays.asList(Aggregates.match(Filters.and(Filters.eq("deviceId", "M201610011949"),
						Filters.ne("upsideId", null), Filters.gt("createTime", start),
		...
		for (Document document : iterable) {
			...
			if(collectionName.equals("obdCarsUpsideDoc")){
				System.out.println(document.get("gpsTime"));
				item.put("gpsTime", document.get("gpsTime"));
			}
